...
def setup_signals():
    ...
    user_logged_in.connect(
        successful_login,
        dispatch_uid="successful_login"
    )
    user_logged_out.connect(
        successful_logout,
        dispatch_uid="successful_logout"
    )
    user_login_failed.connect(
        failed_login,
        dispatch_uid="failed_login"
    )
    post_save.connect(
        adminlog_post_save,
        sender=LogEntry,
        dispatch_uid="adminlog_post_save"
    )
    m2m_changed.connect(
        log_m2m_change,
        sender=User.groups.through,
        dispatch_uid="log_m2m_changed_user_groups"
    )
    m2m_changed.connect(
        log_m2m_change,
        sender=User.user_permissions.through,
        dispatch_uid="log_m2m_changed_user_permissions"
    )
    m2m_changed.connect(
        log_m2m_change,
        sender=Group.permissions.through,
        dispatch_uid="log_m2m_changed_groups_permissions"
    )
